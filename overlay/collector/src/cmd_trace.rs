...
use crate::analyzers::{
    AlertConfig, AlertSink, AuthHeaderRemover, HTTPDecompressor, HTTPFilter, HTTPParser,
    MaterializingAnalyzer, PoisonAnalyzer, SSEProcessor, SSLFilter, TimestampNormalizer,
};
...
pub(crate) struct TraceConfig {
    ...
    /// MemGuard: capture memory-store writes (memwrite probe) and scan poison.
    pub(crate) memwrite: bool,
    /// Path to the memwrite probe binary (required when memwrite is enabled).
    pub(crate) memwrite_binary_path: Option<String>,
    /// Memory-store filename prefix the memwrite probe watches.
    pub(crate) memwrite_prefix: Option<String>,
    /// MemGuard: external poison signature rules file (one per line, # comment).
    pub(crate) memwrite_rules: Option<String>,
    /// MemGuard alerts: WeCom bot webhook key.
    pub(crate) alert_wecom: Option<String>,
    /// MemGuard alerts: ServerChan send key.
    pub(crate) alert_serverchan: Option<String>,
    /// Override alert API host (echo endpoint for联调).
    pub(crate) alert_base: Option<String>,
    /// Print alerts instead of sending.
    pub(crate) alert_dry_run: bool,
    /// Alert batch push window in seconds.
    pub(crate) alert_interval_secs: u64,
...
pub(crate) fn build_trace_agent_with_view(
    ...
    if cfg.memwrite {
        let memwrite_bin = cfg
            .memwrite_binary_path
            .as_deref()
            .ok_or_else(|| RunnerError::from("--memwrite requires --memwrite-path"))?;
        let prefix = cfg
            .memwrite_prefix
            .clone()
            .unwrap_or_else(|| "ehragent_memory".to_string());
        let mut memwrite_runner = BinaryRunner::memwrite(memwrite_bin);
        memwrite_runner =
            memwrite_runner.with_args(&["--prefix".to_string(), prefix.clone()]);
        // TimestampNormalizer converts ns-since-boot to epoch ms; PoisonAnalyzer
        // scans the windows and compensates via the written file range;
        // AlertSink batches poison-hit events to WeCom/ServerChan on a window.
        memwrite_runner = memwrite_runner.add_analyzer(Box::new(TimestampNormalizer::new()));

        // External rules: one signature per line, '#' comments / blanks skipped.
        // Empty file falls back to the built-in 15-signature default.
        let rules = match &cfg.memwrite_rules {
        ...
        memwrite_runner =
            memwrite_runner.add_analyzer(Box::new(PoisonAnalyzer::new(&prefix).with_rules(rules)));

        let mut alert_cfg = AlertConfig::new().interval_secs(cfg.alert_interval_secs);
        if let Some(k) = &cfg.alert_wecom {
            alert_cfg = alert_cfg.wecom(k.clone());
        }
        if let Some(k) = &cfg.alert_serverchan {
            alert_cfg = alert_cfg.serverchan(k.clone());
        }
        if let Some(b) = &cfg.alert_base {
            alert_cfg = alert_cfg.base(b.clone());
        }
        if cfg.alert_dry_run {
            alert_cfg = alert_cfg.dry_run(true);
        }
        if alert_cfg.enabled() {
            memwrite_runner = memwrite_runner.add_analyzer(Box::new(AlertSink::new(alert_cfg)));
        }

        agent = agent.add_runner(Box::new(memwrite_runner));
    }
    ...
    if !cfg.ssl && !cfg.process && !cfg.stdio && !cfg.system && !cfg.memwrite {
        return Err(
            "At least one monitoring type must be enabled (--ssl, --process, --stdio, --system, or --memwrite)"
                .into(),
        );
    }
