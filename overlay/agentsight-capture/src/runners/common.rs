...
impl BinaryRunner {
    ...
    /// MemGuard memory-store write probe: emits `MEM_WRITE` JSONL events
    /// with a `timestamp` (ns since boot) field consumed by TimestampNormalizer.
    pub fn memwrite(binary_path: impl AsRef<Path>) -> Self {
        Self::new("MemWrite", "memwrite", "timestamp", binary_path)
    }
