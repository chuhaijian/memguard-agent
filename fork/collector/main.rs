...
enum Commands {
    ...
    Record {
        ...
        /// MemGuard: capture memory-store writes and flag poison signatures
        #[arg(long)]
        memwrite: bool,
        /// Path to the memwrite probe binary (required with --memwrite)
        #[arg(long)]
        memwrite_path: Option<String>,
        /// Memory-store filename prefix the memwrite probe watches
        #[arg(long, default_value = "ehragent_memory")]
        memwrite_prefix: String,
        /// MemGuard: external poison signature rules file (one per line, # comment)
        #[arg(long = "poison-rules")]
        memwrite_rules: Option<String>,
        /// MemGuard alert: WeCom bot webhook key
        #[arg(long)]
        alert_wecom: Option<String>,
        /// MemGuard alert: ServerChan send key
        #[arg(long)]
        alert_serverchan: Option<String>,
        /// Override alert API host (echo endpoint for联调)
        #[arg(long)]
        alert_base: Option<String>,
        ...
        Commands::Record {
            ...
            run_trace(
                ...
                TraceConfig {
                    ...
                    memwrite: *memwrite,
                    memwrite_binary_path: memwrite_path.clone(),
                    memwrite_prefix: Some(memwrite_prefix.clone()),
                    memwrite_rules: memwrite_rules.clone(),
                    alert_wecom: alert_wecom.clone(),
                    alert_serverchan: alert_serverchan.clone(),
                    alert_base: alert_base.clone(),
                    alert_dry_run: *alert_dry_run,
                    alert_interval_secs: *alert_interval,
                    db_path,
                    server: !*no_server,
