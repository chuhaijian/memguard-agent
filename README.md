...
LLM Agent 的长期记忆（向量库、SQLite 记忆库、会话摘要文件等）存在一类隐蔽攻击面：**记忆投毒（Memory Poisoning / Indirect Prompt Injection）**。攻击者在 Agent 可读取的内容中植入带有"信任背书"语义的片段——例如 `trusted source`、`免验证`、`可直接导入`——一旦被写入记忆库，后续会话会无条件继承这些指令，形成跨会话的持久化后门（学术上称为 FARMA / MINJA 类攻击）。
...
本仓库提供的是**叠加在 AgentSight 之上的改动文件层**，`fork/` 下的目录结构与上游仓库一一对应。
...
# 2. 叠加本仓库改动（保持相对路径一致）
cp -R /path/to/memguard-agent/fork/* ./
...
./target/release/agentsight record \
  ...
  --poison-rules /path/to/poison-rules.txt \
  -c python3 \
...
```bash
install -m 0644 fork/systemd/memguard.env.example /etc/memguard.env   # 默认 dry_run=1
install -m 0755 fork/systemd/run-memguard.sh      /opt/agentsight/run-memguard.sh
install -m 0644 fork/systemd/memguard.service     /etc/systemd/system/memguard.service
...
```
memguard-agent/
├── fork/                      # 叠加到 AgentSight 的改动文件（保持上游相对路径）
│   ├── memwrite.bpf.c/.c/.h   # eBPF 探针内核态逻辑 + libbpf loader
│   ├── poison.rs              # PoisonAnalyzer：签名匹配 + FS 补偿
│   ├── alert_sink.rs          # AlertSink：告警聚合与推送（hyper）
│   ├── ext-analysis/          # analyzer 注册
│   ├── agent-sight-capture/   # BinaryRunner 构造器
│   ├── collector/             # CLI 接线
│   └── systemd/               # 常驻服务三件套
